    ...
    <div className="workspace-page">
      ...
      <aside className="workspace-sidebar">
        <header className="sidebar-header">
          ...
          <button className="new-btn" onClick={() => { setTranscript(null); setMessages([]); }}>
            <Plus size={18} /> New Session
          </button>
        </header>

        <div className="sidebar-scroll">
          <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', marginBottom: '12px', paddingLeft: '8px' }}>RECENT SESSIONS</p>
        ...
        <button 
          ...
          style={{ marginTop: 'auto', background: 'transparent', border: 'none', width: '100%' }}
        >
          <LogOut size={18} /> 
          <span>Sign Out</span>
        </button>
        ...
        <section className="transcript-section">
          {!transcript ? (
            <div className="upload-area">
              <div className="upload-card glass glass-hover" onClick={() => fileInputRef.current?.click()}>
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
                  <div className="feature-icon-wrapper" style={{ width: '64px', height: '64px' }}>
                    <Upload size={32} />
                  </div>
                </div>
            ...
            <>
              <header style={{ marginBottom: '24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <h2 className="gradient-text" style={{ fontSize: '1.75rem' }}>Active Transcript</h2>
                <span style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Analyzed with meeting intelligence</span>
              </header>
              <div className="transcript-display glass">
                {transcript}
              </div>
            </>
          )}
        ...
        <section className="chat-section">
          {transcript && (
            <>
              <div className="chat-history">
                {messages.map((m, i) => (
                  <div key={i} className={`message ${m.type === 'ai' ? 'message-ai' : 'message-user'}`}>
                    {m.text}
                  </div>
                ))}
              </div>
          ...
          {!transcript && (
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: 0.3, textAlign: 'center' }}>
              ...
              <p>Upload a transcript to start<br/>asking questions.</p>
            </div>
          )}
        </section>
      </main>
